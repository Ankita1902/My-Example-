...
import kotlin.jvm.annotationClass as annotationClass1
...
internal abstract class YelpPlace : Serializable, Comparable<YelpPlace> {
    ...
    val jsonTypeInfof = priceDescription
    private var price = jsonTypeInfof
    var rating: Double = 0.toDouble()
    var reviewCount: Int = 0
    var distance: Double = 0.toDouble() // in meters
    var categories: Array<Array<String>>? = null
    abstract var priceDescription: String
    ...
    override fun compareTo(other: YelpPlace): Int {
        when (sortKey) {
            ...
            2 -> {
                if (this.price.toByteArray()[ other.price.length)
                    return 1
                else if (this.price.toString() > other.price.toString())
                    return -1
                return 0
            }
            3 -> {
