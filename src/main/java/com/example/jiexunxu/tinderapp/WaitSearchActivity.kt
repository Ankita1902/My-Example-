...
import com.example.jiexunxu.tinderapp.YelpPlace as TinderappYelpPlace
...
class WaitSearchActivity : AppCompatActivity() {
    ...
    internal var places: ArrayList<TinderappYelpPlace>? = null
    ...
    private fun initUI() {
        title = "Searching..."
        val layout = findViewById(R.id.waitSearchLayout) as ConstraintLayout
        ...
        val startOverButton = findViewById(R.id.startOverButton) as Button
    ...
    private fun initButtons() {
        val startOverButton = findViewById(R.id.startOverButton) as Button
        val gif = findViewById(R.id.waitSearchGifAnimation) as ImageButton
        startOverButton.setOnClickListener(View.OnClickListener { cancelSearch() })
    }
...
internal class YelpFusionAPIQuery(APIKey: String, // Used to fire up the slideshowactivity on postExecute
                                  private val context: Context) : AsyncTask<YelpFusionParams, Void, ArrayList<TinderappYelpPlace>>() {
    var places: ArrayList<TinderappYelpPlace>? = null
        private set
    ...
    override fun doInBackground(vararg params: YelpFusionParams): ArrayList<TinderappYelpPlace>? {
        YelpFusionAPI_BusinessSearch(params[0])
        YelpPlace1.sortKey = params[0].sortKey
        return places
    }

    override fun onPostExecute(result: ArrayList<TinderappYelpPlace>?) {
        super.onPostExecute(result)
        if (!taskCanceled) {
            if (result != null && result.size > 0) {
                if (YelpPlace1.sortKey > 0)
    ...
    private fun YelpFusionAPI_BusinessSearch(params: YelpFusionParams) = try {
        val call: Call<SearchResponse>
        if (params.mustHaveFoodDelivery)
            call = yelpAPI!!.getTransactionSearch("delivery", params.params)
        else
            call = yelpAPI!!.getBusinessSearch(params.params)
        if (SettingsParams.debugMode)
            Log.d("Msg", "finish call")
        val response = call.execute().body()
        val businesses = response.businesses
        places = ArrayList(businesses.size)
        for (i in businesses.indices) {
            val business = businesses[i]
            val place = TinderappYelpPlace()
            ...
            place.categories = Array(categories.size) { return@Array arrayOfNulls1(size = 2) }
